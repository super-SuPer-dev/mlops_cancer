...
from sklearn.metrics import accuracy_score, roc_auc_score  # ✅ เพิ่ม roc_auc_score
...
def train_evaluate_register(preprocessing_run_id, C=1.0):
    ...
    ROC_AUC_THRESHOLD = 0.98  # ✅ เพิ่มเกณฑ์ roc_auc
    MODEL_NAME = "cancer-classifier-prod"  # ✅ เปลี่ยนชื่อโมเดล
    ...
    with mlflow.start_run(run_name=f"logistic_regression_C_{C}"):
        ...
        # ✅ binary classification ต้องใช้ predict_proba คำนวณ roc_auc
        #    [:, 1] = ความน่าจะเป็นของคลาสบวก (malignant = 1)
        ...
        mlflow.log_metric("roc_auc", roc_auc)  # ✅ log roc_auc เพิ่ม
        model_info = mlflow.sklearn.log_model(
            sk_model=pipeline,
            name="cancer_classifier_pipeline",  # ✅ เปลี่ยนชื่อ artifact ให้สอดคล้อง
            input_example=X_train.head(5)
        )

        # 5. ตรวจสอบและลงทะเบียนโมเดล
        # ✅ gate ผ่านทั้งสองเกณฑ์ จึงจะลงทะเบียน
        if acc >= ACCURACY_THRESHOLD and roc_auc >= ROC_AUC_THRESHOLD:
            print(f"Model meets thresholds (acc={acc:.4f}, roc_auc={roc_auc:.4f}). Registering model...")
            ...
            print(f"Model below thresholds (acc={acc:.4f}, roc_auc={roc_auc:.4f}). Not registering.")
