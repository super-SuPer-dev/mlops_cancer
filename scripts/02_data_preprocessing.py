...
def preprocess_data(test_size=0.25, random_state=42):
    ...
    with mlflow.start_run() as run:
        ...
        # ค้นหา run ล่าสุดจาก experiment ของขั้นตอน preprocessing
        runs = mlflow.search_runs(
            experiment_names=["Breast Cancer - Data Preprocessing"],
            filter_string='tags.ml.step = "data_preprocessing"',
        ...
        run_id = latest["run_id"]                      # hex 32 ตัว
        training_rows = latest["metrics.training_set_rows"]

        print(f"Preprocessing Run ID: {run_id}")
        print(f"training_set_rows: {training_rows}")
        print("-" * 50)
